...
class DiffPoolGNN(GNN):
    def __init__(self, num_tasks=1, num_layers=5, emb_dim=300, gnn_type='gin',
                 ...
                 graph_pooling="sum"):
        super(DiffPoolGNN, self).__init__(num_tasks, num_layers, emb_dim,
                                          gnn_type,
                                          virtual_node, residual, drop_ratio,
                                          JK, graph_pooling)
    ...
    def forward(self, g, x, edge_attr):
        ...
        # 4. GCN:
        h_node = self.gcn_forward_tensorized(h_node, adj, [self.gc_after_pool],
                                             True)
        #         print(h_node.shape)
